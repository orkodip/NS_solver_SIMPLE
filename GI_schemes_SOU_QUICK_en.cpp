...
void PVC :: CD_U(int j, int i, float F_e, float F_w, float F_n, float F_s, float D_e, float D_w, float D_n, float D_s)
{
	...
	A_E = D_e-A_1e*MAX(F_e,0.0)+A_2e*MAX(0.0,-F_e);

	A_W = D_w+A_2w*MAX(F_w,0.0)-A_1w*MAX(0.0,-F_w);

	A_N = D_n-A_1n*MAX(F_n,0.0)+A_2n*MAX(0.0,-F_n);

	A_S = D_s+A_2s*MAX(F_s,0.0)-A_1s*MAX(0.0,-F_s);

	A_P = A_E+A_W+A_N+A_S+F_e*(A_1e+A_2e)-F_w*(A_1w+A_2w)+F_n*(A_1n+A_2n)-F_s*(A_1s+A_2s);
}
...
void PVC :: CD_geo_TH(int j, int i,float *A_1w,float *A_2w,float *A_1e,float *A_2e,float *A_1n,float *A_2n,float *A_1s,float *A_2s,float F_e, float F_w, float F_n, float F_s)
{
	float D_1w=0.0,D_2w=0.0, D_1e=0.0,D_2e=0.0, D_1n=0.0,D_2n=0.0, D_1s=0.0,D_2s=0.0;

	D_1w = (U[0][i-1][j]-TH[0][i-1][j])*CHK(F_w)+(TH[0][i][j]-U[0][i-1][j])*CHK(-F_w);

	D_2w = (TH[0][i][j]-U[0][i-1][j])*CHK(F_w)+(U[0][i-1][j]-TH[0][i-1][j])*CHK(-F_w);

	*A_1w = D_1w/(D_1w+D_2w); *A_2w = D_2w/(D_1w+D_2w);
//------------------------------------------------------------------------------------------------------------------------------
	D_1e = (U[0][i][j]-TH[0][i][j])*CHK(F_e)+(TH[0][i+1][j]-U[0][i][j])*CHK(-F_e);

	D_2e = (TH[0][i+1][j]-U[0][i][j])*CHK(F_e)+(U[0][i][j]-TH[0][i][j])*CHK(-F_e);

	*A_1e = D_1e/(D_1e+D_2e); *A_2e = D_2e/(D_1e+D_2e);
//---------------------------------------------------------------------------------------------------------------------------
	D_1n = (V[1][i][j]-TH[1][i][j])*CHK(F_n)+(TH[1][i][j+1]-V[1][i][j])*CHK(-F_n);

	D_2n = (TH[1][i][j+1]-V[1][i][j])*CHK(F_n)+(V[1][i][j]-TH[1][i][j])*CHK(-F_n);

	*A_1n = D_1n/(D_1n+D_2n); *A_2n = D_2n/(D_1n+D_2n);
//---------------------------------------------------------------------------------------------------------------------------
	D_1s = (V[1][i][j-1]-TH[1][i][j-1])*CHK(F_s)+(TH[1][i][j]-V[1][i][j-1])*CHK(-F_s);

	D_2s = (TH[1][i][j]-V[1][i][j-1])*CHK(F_s)+(V[1][i][j-1]-TH[1][i][j-1])*CHK(-F_s);

	*A_1s = D_1s/(D_1s+D_2s); *A_2s = D_2s/(D_1s+D_2s);
}

void PVC :: CD_TH(int j, int i, float F_e, float F_w, float F_n, float F_s, float D_e, float D_w, float D_n, float D_s)
{
	float A_1w=0.0,A_2w=0.0, A_1e=0.0,A_2e=0.0, A_1n=0.0,A_2n=0.0, A_1s=0.0,A_2s=0.0;

	CD_geo_TH(j,i,&A_1w,&A_2w,&A_1e,&A_2e,&A_1n,&A_2n,&A_1s,&A_2s,F_e,F_w,F_n,F_s);
...
void PVC :: QUICK_geo_TH(int j,int i,float *G_1w,float *G_3w,float *G_1e,float *G_3e,float *G_1n,float *G_3n,float *G_1s,float *G_3s,float *K_1w,float *K_3w,float *K_1e,float *K_3e,float *K_1n,float *K_3n,float *K_1s,float *K_3s,float *r_w,float *r_e,float *r_n,float *r_s,float F_e, float F_w, float F_n, float F_s)
{
	float D_1w=0.0,D_2w=0.0,D_3w=0.0, D_1e=0.0,D_2e=0.0,D_3e=0.0, D_1n=0.0,D_2n=0.0,D_3n=0.0, D_1s=0.0,D_2s=0.0,D_3s=0.0;
	...
	D_3w = (U[0][i-1][j]-TH[0][i-2][j])*CHK(F_w)+(TH[0][i+1][j]-U[0][i-1][j])*CHK(-F_w);
...
//---------------------------------------------------------------------------------------------------------------------------
	...
	D_3e = (U[0][i][j]-TH[0][i-1][j])*CHK(F_e)+(TH[0][i+2][j]-U[0][i][j])*CHK(-F_e);
...
//---------------------------------------------------------------------------------------------------------------------------
	...
	D_3n = (V[1][i][j]-TH[1][i][j-1])*CHK(F_n)+(TH[1][i][j+2]-V[1][i][j])*CHK(-F_n);
...
//---------------------------------------------------------------------------------------------------------------------------
	...
	D_3s = (V[1][i][j-1]-TH[1][i][j-2])*CHK(F_s)+(TH[1][i][j+1]-V[1][i][j-1])*CHK(-F_s);
...
void PVC :: QUICK_TH(int j, int i, float F_e, float F_w, float F_n, float F_s, float D_e, float D_w, float D_n, float D_s)
{
	float G_1w=0.0,G_3w=0.0, G_1e=0.0,G_3e=0.0, G_1n=0.0,G_3n=0.0, G_1s=0.0,G_3s=0.0;
	float K_1w=0.0,K_3w=0.0, K_1e=0.0,K_3e=0.0, K_1n=0.0,K_3n=0.0, K_1s=0.0,K_3s=0.0;
	float r_w=0.0,r_e=0.0,r_n=0.0,r_s=0.0;

	QUICK_geo_TH(j,i,&G_1w,&G_3w,&G_1e,&G_3e,&G_1n,&G_3n,&G_1s,&G_3s,&K_1w,&K_3w,&K_1e,&K_3e,&K_1n,&K_3n,&K_1s,&K_3s,&r_w,&r_e,&r_n,&r_s,F_e,F_w,F_n,F_s);
	...
	S = -r_e*(TH[2][i+1][j]-K_3e*TH[2][i][j]+K_1e*TH[2][i-1][j])*MAX(F_e,0.0)+r_w*(TH[2][i][j]-K_3w*TH[2][i-1][j]
		+K_1w*TH[2][i-2][j])*MAX(F_w,0.0)-r_n*(TH[2][i][j+1]-K_3n*TH[2][i][j]+K_1n*TH[2][i][j-1])*MAX(F_n,0.0)
		+r_s*(TH[2][i][j]-K_3s*TH[2][i][j-1]+K_1s*TH[2][i][j-2])*MAX(F_s,0.0)+r_e*(TH[2][i][j]-K_3e*TH[2][i+1][j]
		+K_1e*TH[2][i+2][j])*MAX(-F_e,0.0)-r_w*(TH[2][i-1][j]-K_3w*TH[2][i][j]+K_1w*TH[2][i+1][j])*MAX(-F_w,0.0)
		+r_n*(TH[2][i][j]-K_3n*TH[2][i][j+1]+K_1n*TH[2][i][j+2])*MAX(-F_n,0.0)-r_s*(TH[2][i][j-1]-K_3s*TH[2][i][j]
		+K_1s*TH[2][i][j+1])*MAX(-F_s,0.0)-G_1w*TH[2][i-2][j]*MAX(F_w,0.0)-G_1s*TH[2][i][j-2]*MAX(F_s,0.0)-G_1e*TH[2][i+2][j]*MAX(-F_e,0.0)
		-G_1n*TH[2][i][j+2]*MAX(-F_n,0.0);
}
